...
import { ArrowRight, Save, FileText, Download } from "lucide-react"
...
export default function HomeworkSubmissionReviewPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/homework-submissions/${id}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const maxScore = submission.answers.reduce((sum, a) => sum + a.homeworkquestion.points, 0)
      const res = await fetch(`/api/admin/homework-submissions/${id}`, {
        method: "PATCH",
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-2">
        ...
        <span className="font-medium">
          {submission.homework.titleAr} — {submission.user.name}
        </span>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          {submission.answers.map((a) => (
            ...
            >
              <p className="font-medium">{a.homeworkquestion.questionTextAr}</p>
              <p className="text-sm text-muted-foreground">
                ({a.homeworkquestion.type}) — {a.homeworkquestion.points} نقطة
              </p>

              {a.homeworkquestion.type === "file" && a.filePath ? (
                <div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="space-y-2">
                  <Label>النقاط (0 - {a.homeworkquestion.points})</Label>
                  <Input
                    type="number"
                    min={0}
                    max={a.homeworkquestion.points}
