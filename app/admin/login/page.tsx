...
import { setAdminUser } from "@/lib/admin-auth"
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!["admin", "super_admin", "instructor"].includes(user.role)) {
        setError("غير مصرح لك بالدخول للوحة التحكم")
        return
      }
    ...
    <div className="relative flex min-h-screen" dir="rtl">
      ...
      <div className="relative hidden w-1/2 items-center justify-center lg:flex">
        <div className="relative z-10 px-16 text-center">
          <motion.div className="relative mx-auto mb-10" style={{ width: 160, height: 160 }}>
            <motion.div
          ...
          >
            ...
            <p className="mx-auto max-w-sm text-lg leading-relaxed text-white/50">
              تسجيل دخول المدربين والمسؤولين
            </p>
          </motion.div>

          <div className="mt-12 flex items-center justify-center gap-8">
            {[
              { icon: BookOpen, label: "إدارة الدورات" },
              { icon: Zap, label: "تقارير وتحصيل" },
              { icon: Shield, label: "صلاحيات آمنة" },
            ].map((item, i) => (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 20 }}
        ...
        >
          <div className="relative overflow-hidden rounded-3xl border border-white/[0.08] bg-white/[0.03] p-8 shadow-2xl shadow-black/20 backdrop-blur-xl sm:p-10">
            ...
            >
              ...
              <p className="text-sm text-white/40">المدربين والمسؤولين</p>
            </motion.div>
            ...
            <form className="space-y-5" onSubmit={handleLogin}>
              ...
              <motion.div custom={2} variants={formFieldVariants} initial="hidden" animate="visible" className="flex items-center gap-2">
                <Checkbox
                  id="remember"
                  checked={rememberMe}
              ...
              {error && (
                <motion.p
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="text-sm text-red-500 font-medium text-center bg-red-50 rounded-xl py-2.5 px-4 border border-red-100"
                >
                  {error}
                </motion.p>
              )}
            ...
            >
              تسجيل كمدرب؟{" "}
              <Link href="/admin/register" className="font-semibold text-[#2563EB] transition-colors hover:text-[#0EA5E9]">
                إنشاء حساب مدرب
              </Link>
            </motion.p>
          </div>
